...
function slidenavHighlight($next) {
	$("#slidenav a").removeClass("cur");
	var currentNum = $("#slideshow div").index($next);
	var $highlightNav = $("#slidenav a:first");
...
$(function() {
   ...
   // 鼠标悬停时停止换图
	...
	for( i = 0; i < $imgNum; i++ ){
		$(".slidenavcon").append('<a href="#"><span>&#149;</span></a>');
	}
	$("#slidenav a:first").addClass("cur");	
	
	$("#slidenav a").bind("click",function(){
		clearInterval(autoSlide);
		var slideindex = $("#slidenav a").index(this);
		if( !($(".current").is(":animated")))	
